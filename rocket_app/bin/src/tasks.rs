...
pub async fn get_task(id: i32, pool: &State<DbPool>) -> Option<Json<Task>> {
    ...
    Task::read(&mut conn, id).ok().flatten().map(Json)
}
...
#[delete("/tasks/<id>")]
pub async fn delete_task(id: i32, pool: &State<DbPool>) -> Result<Json<usize>, Status> {
    let mut conn = pool.get().map_err(|_| Status::InternalServerError)?;

    match Task::delete(&mut conn, id) {
        Ok(count) => Ok(Json(count)),
        Err(e) => {
            eprintln!("Delete error: {}", e);
            Err(Status::Conflict)  // 409 Conflict for FK violations
            }
        }
    }
